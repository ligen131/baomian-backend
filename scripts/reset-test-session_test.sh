...
grep -qi 'INSERT INTO conversation_runs' "$tmp/sql-2"
grep -qi 'INSERT INTO memory_cards' "$tmp/sql-2"
grep -qi "INTERVAL '3 days'" "$tmp/sql-2"
grep -qi "INTERVAL '2 days'" "$tmp/sql-2"
grep -qi "INTERVAL '1 day'" "$tmp/sql-2"
grep -Fq '今天完成了几件一直惦记的小事，心里松了一些。' "$tmp/sql-2"
grep -Fq '工作还有一些没有收尾，担心明天会来不及。' "$tmp/sql-2"
grep -Fq '明天有新的安排，期待里也带着一点紧张。' "$tmp/sql-2"
grep -qi 'ON CONFLICT' "$tmp/sql-2"
